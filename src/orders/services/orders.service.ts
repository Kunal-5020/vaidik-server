  ...
  async processRefundRequest(
    ...
    try {
      ...
      if (processData.action === 'approve') {
        ...
        const refundPercentage = processData.refundPercentage || 100;
        ...
        order.refundRequest.adminNotes = processData.adminNotes;
        ...
        // Process actual refund - credit back to user wallet
        try {
          ...
          // TODO: Deduct from astrologer earnings
          // await this.earningsService.deductEarnings(
          //   order.astrologerId.toString(),
          //   refundAmount * 0.8, // Deduct astrologer's 80% share
          //   'refund',
          //   orderId
          // );

        } catch (walletError: any) {
          throw new InternalServerErrorException(`Refund processing failed: ${walletError.message}`);
        }
        ...
        order.refundRequest.processedBy = this.toObjectId(adminId);
        order.refundRequest.adminNotes = processData.adminNotes;
        order.refundRequest.rejectionReason = processData.rejectionReason;
        ...
        this.logger.log(`Refund rejected: ${orderId} | Reason: ${processData.rejectionReason} | Admin: ${adminId}`);
